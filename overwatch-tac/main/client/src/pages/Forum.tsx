...
export default function Profile({ profileId, currentUser }: { profileId: string, currentUser: any }) {
  ...
  const fetchProfile = async () => {
    const { data } = await supabase.from("Users").select("*").eq("user_id", profileId).maybeSingle();
    setProfile(data);
  ...
  const AuthorHeader = ({ user, userId, createdAt }: any) => (
    <div style={{ display: "flex", alignItems: "center", gap: 12 }}>
      <NavLink to={`/profile/${userId}`}>
        <img src={user?.profile_image_link || DEFAULT_AVATAR} style={{ width: 36, height: 36, borderRadius: "50%" }} />
      </NavLink>
      <div>
            ...
            <div key={reply.reply_id} style={{ marginTop: 12, background: "#111", padding: 14, borderRadius: 12, border: '1px solid #222' }}>
              ...
              <div style={{ display: 'flex', gap: 8 }}>
                <button onClick={async () => {
                  ...
                  fetchPosts();
                }} style={{ background: isLiked ? "#3b82f633" : "#222", color: 'white', padding: '4px 10px', borderRadius: 6 }}>👍 {reply.Reply_Likes.length}</button>
                <button onClick={async () => {
                  ...
                  fetchPosts();
                }} style={{ background: isDisliked ? "#ef444433" : "#222", color: 'white', padding: '4px 10px', borderRadius: 6 }}>👎 {reply.Reply_Dislikes.length}</button>
      ...
      {profile && (
        <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 30 }}>
          <div style={{ display: 'flex', alignItems: 'center', gap: 16 }}>
            <img src={profile.profile_image_link || DEFAULT_AVATAR} style={{ width: 64, height: 64, borderRadius: '50%' }} />
            <div>
              <h2 style={{ margin: 0 }}>{profile.username}</h2>
              <p style={{ margin: 0, fontSize: 12, color: '#888' }}>{profile.email}</p>
            </div>
          </div>
          {currentUser?.id !== profileId && (
            <button onClick={toggleFollow} style={{ padding: "8px 18px", borderRadius: 8, background: "#3b82f6", color: 'white', border: 'none', cursor: 'pointer' }}>
          ...
          <div key={post.post_id} style={{ background: '#0a0a0a', padding: 24, borderRadius: 12, border: '1px solid #1a1a1a', marginBottom: 20 }}>
            ...
            <div style={{ display: 'flex', gap: 12 }}>
              <button onClick={async () => {
                ...
                fetchPosts();
              }} style={{ background: isLiked ? "#3b82f633" : "#1a1a1a", color: 'white', padding: '6px 14px', borderRadius: 8 }}>👍 {post.Post_Likes.length}</button>
              ...
              <button onClick={async () => {
                ...
                fetchPosts();
              }} style={{ background: isDisliked ? "#ef444433" : "#1a1a1a", color: 'white', padding: '6px 14px', borderRadius: 8 }}>👎 {post.Post_Dislikes.length}</button>
            ...
            {isExpanded && (
              <div style={{ marginTop: 16, borderTop: '1px solid #1a1a1a', paddingTop: 10 }}>
                ...
                <div style={{ marginTop: 12, display: 'flex', gap: 8 }}>
                  <input type="text" placeholder="Write a reply..." value={replyText[post.post_id] || ""} onChange={e => setReplyText({ ...replyText, [post.post_id]: e.target.value })} style={{ flex: 1, padding: 10, borderRadius: 8, border: '1px solid #222', background: '#000', color: 'white' }} />
                  <button onClick={() => {
                  ...
                  }} style={{ padding: '8px 18px', borderRadius: 8, background: '#3b82f6', border: 'none', color: 'white', cursor: 'pointer', fontWeight: 'bold' }}>Reply</button>
                </div>
              </div>
            )}
          </div>
