...
const TacMap: React.FC = () => {
  ...
  const loadSavedStrategy = async (id: string) => {
    ...
    if (save) {
      ...
      if (assets) {
        setMarkers(assets.map((a: any) => {
          const hero = heroAssets.find(h => h.asset_id === a.asset_id);
          return {
            id: Math.random(), x: a.x_position, y: a.y_position, team: a.hero_team as Team,
      ...
      if (drawing?.path && canvasRef.current) {
        const img = new Image();
        img.crossOrigin = "anonymous"; // Prevents "Tainted Canvas" errors when saving again
        img.onload = () => {
  ...
  const addMarker = (newMarker: Marker) => {
    setMarkers(prev => {
      const currentTeamCount = prev.filter(m => m.team === newMarker.team).length;
      if (currentTeamCount >= 5) return prev;
      if (newMarker.heroName && prev.some(m => m.heroName === newMarker.heroName && m.team === newMarker.team)) return prev;
  ...
  const triggerSaveFlow = () => {
    ...
    activeSaveId ? finalizeSave() : setIsNameModalOpen(true);
  };
  ...
  const finalizeSave = async () => {
    ...
    try {
      ...
      if (!currentId) {
        const { data: header } = await supabase.from("Saved_Maps").insert([{
        ...
        await supabase.from("Saved_Maps").update({ description }).eq("save_id", currentId);
      ...
      if (markers.length > 0) {
        await supabase.from("Map_Assets").insert(markers.map(m => ({
          save_id: currentId,
          asset_id: m.type === "asset" ? heroAssets.find(h => h.name === m.heroName)?.asset_id : null,
          x_position: Math.round(m.x), y_position: Math.round(m.y), hero_team: m.team 
        })));
      }

      // CRITICAL FIX FOR DRAWING UPDATE
      if (canvasRef.current) {
        // We force a small delay or ensure the context is flushed
        const drawingData = canvasRef.current.toDataURL("image/png");
        
        // Use UPSERT with a primary key constraint to overwrite
        const { error: upsertError } = await supabase.from("Map_Drawings").upsert({
          save_id: currentId,
          path: drawingData,
  ...
  const getCoords = (e: React.MouseEvent) => {
    ...
    return { x: (e.clientX - rect.left) * (1000 / rect.width), y: (e.clientY - rect.top) * (600 / rect.height) };
  };
  ...
  return (
    <div style={{ padding: "20px", color: "white", backgroundColor: "#111", minHeight: "100vh" }}>
        ...
        <div style={{ flex: 1 }}>
          <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: "10px" }}>
            <div style={{ display: "flex", gap: "10px" }}>
              ...
              <button onClick={triggerSaveFlow} disabled={isSaving} style={{ padding: "8px 16px", background: isSaving ? "#444" : "#28a745", border: "none", borderRadius: "4px", color: "white", cursor: "pointer", fontWeight: "bold" }}>
          ...
          <div 
            ...
            onDrop={(e) => {
              const asset = JSON.parse(e.dataTransfer.getData("assetData"));
          ...
          >
            <canvas 
              ...
              onMouseUp={() => {
                setIsDrawing(false);
                canvasRef.current?.getContext("2d")?.closePath(); // Flush the path
              }}
              style={{ position: "absolute", top: 0, left: 0, width: "100%", height: "100%", cursor: "crosshair", zIndex: 2 }} 
            />
            {markers.map(m => (
              <div key={m.id} style={{ position: "absolute", left: m.x - 20, top: m.y - 20, width: "40px", height: "40px", zIndex: 5, pointerEvents: "none" }}>
                <img src={m.iconUrl} style={{ width: "100%", borderRadius: "50%", border: `2px solid ${m.team === "ally" ? "#007bff" : "#dc3545"}`, background: "#222" }} />
              </div>
            ))}
          </div>
        </div>
      ...
      <CustomModal isOpen={isNameModalOpen} title="Name Your Strategy" onConfirm={finalizeSave} onCancel={() => setIsNameModalOpen(false)}>
        <input type="text" value={newStrategyName} onChange={(e) => setNewStrategyName(e.target.value)} placeholder="e.g. Volskaya Dive" style={{ width: "100%", padding: "12px", background: "#000", border: "1px solid #e60082", color: "white", borderRadius: "4px", outline: "none", textAlign: "center" }} />
      </CustomModal>
