...
/** * TYPES & INTERFACES 
 * Defines the shape of map data, assets, and the drawing/history system.
 */
...
/**
 * REUSABLE COMPONENTS
 * CustomModal and ToolButton provide the UI consistency across the Tactical Map.
 */
...
const UndoIcon = () => (
  <svg width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round">
    <path d="M3 7v6h6"/><path d="M21 17a9 9 0 0 0-15.5-6L3 13"/>
  </svg>
);
const RedoIcon = () => (
  <svg width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round">
    <path d="M21 7v6h-6"/><path d="M3 17a9 9 0 0 1 15.5-6L21 13"/>
  </svg>
);
const TrashIcon = () => (
  <svg width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round">
...
const TacMap: React.FC = () => {
  ...
  /** * DATA FETCHING 
   * Fetches available maps and hero assets on component mount.
   */
  useEffect(() => {
  ...
  /** * HISTORY SYSTEM
   * Handles undo/redo operations by storing snapshots of markers and drawings.
   */
  ...
  /** * KEYBOARD LISTENERS
   * Handles shortcuts for undo, redo, tools, and panning.
   */
  useEffect(() => {
    const handleKeyDown = (e: KeyboardEvent) => {
      ...
      if (modifier && e.key.toLowerCase() === 'z' && !e.shiftKey) { e.preventDefault(); handleUndo(); }
      else if ((modifier && e.key.toLowerCase() === 'y') || (modifier && e.shiftKey && e.key.toLowerCase() === 'z')) { e.preventDefault(); handleRedo(); }
      else if (e.key === 'Delete' || e.key === 'Backspace') { if (selectedElement) { e.preventDefault(); deleteSelectedElement(); } }
    ...
    const handleKeyUp = (e: KeyboardEvent) => { if (e.key === ' ') { setSpacePressed(false); setIsPanning(false); } };
   ...
   * Uses Quadratic Curves for smooth line rendering and handles brush previews.
   */
  const drawCanvas = () => {
  ...
  /** * PERSISTENCE LOGIC
   * Handles loading, saving, and updating strategies in Supabase.
   */
      ...
      if (save) {
        ...
        if (assets) {
          loadedMarkers = assets.map((a: any) => {
            ...
            return {
              id: a.id, x: a.x_position, y: a.y_position,
              team: a.hero_team as Team,
              type: a.asset_id ? "asset" : "player",
              iconUrl: hero?.image_path, heroName: hero?.name,
        ...
        if (drawing?.path) {
          ...
          } catch(e) { console.error("Failed to parse drawings"); }
        }
  ...
  const finalizeSave = async () => {
    ...
    try {
      ...
      // UNIQUE NAME CHECK: Only if it's a new save or the name is changing
      if (!currentId) {
        const { data: existing } = await supabase
          .from("Saved_Maps")
          .select("name")
          .eq("user_id", user.id)
          .eq("name", newStrategyName)
          .maybeSingle();

        if (existing) {
          setIsDuplicateNameModalOpen(true);
          setIsSaving(false);
          return;
        }
      ...
      if (markers.length > 0) {
        await supabase.from("Map_Assets").insert(markers.map(m => ({
          ...
          x_position: Math.round(m.x), y_position: Math.round(m.y), 
          hero_team: m.team 
        })));
      }
   ...
   * Handles clicking, dragging, dropping, and zooming on the tactical map.
   */
  const clearCanvas = () => {
  ...
  const handleDrop = (e: React.DragEvent) => {
    ...
    if ((activeTeam === "ally" ? allyMarkers.length : enemyMarkers.length) >= 5) return;
  ...
  const deleteSelectedElement = () => {
    ...
    if (selectedElement.type === "marker") { nextMarkers = markers.filter(m => m.id !== selectedElement.id); setMarkers(nextMarkers); }
    else if (selectedElement.type === "drawing") { nextDrawings = drawings.filter(d => d.id !== selectedElement.id); setDrawings(nextDrawings); }
  ...
  const isPointNearLine = (px: number, py: number, line: DrawingLine): boolean => {
    ...
    for (let i = 0; i < line.points.length - 1; i++) {
      const x1 = line.points[i].x; const y1 = line.points[i].y;
      const x2 = line.points[i+1].x; const y2 = line.points[i+1].y;
      ...
      if (Math.sqrt((px - xx) ** 2 + (py - yy) ** 2) < threshold) return true;
    }
    return false;
  };
  ...
  const handleMouseDown = (e: React.MouseEvent) => {
    if (!selectedMap) return;
    if (spacePressed || e.button === 1) { e.preventDefault(); setIsPanning(true); return; }
    const { x, y } = getCoords(e);
    if (activeTool === "pen") {
      setIsDrawing(true);
      const newLine: DrawingLine = { id: Date.now(), points: [{ x, y }], color: activeTeam === "ally" ? "#007bff" : "#dc3545", width: brushSize };
      setDrawings(prev => [...prev, newLine]);
      return;
    }
    if (activeTool === "select") {
      const clickedMarker = markers.find(m => Math.sqrt((m.x - x) ** 2 + (m.y - y) ** 2) < 25);
      if (clickedMarker) { setIsDraggingElement(true); setSelectedElement({ type: "marker", id: clickedMarker.id }); return; }
      const clickedDrawing = drawings.find(d => isPointNearLine(x, y, d));
      if (clickedDrawing) {
  ...
  const handleMouseMove = (e: React.MouseEvent) => {
    ...
    if (isDrawing && activeTool === "pen") {
      setDrawings(prev => {
        const updated = [...prev]; const lastLine = updated[updated.length - 1];
    ...
    if (isDraggingElement && selectedElement) {
      if (selectedElement.type === "marker") { setMarkers(prev => prev.map(m => m.id === selectedElement.id ? { ...m, x, y } : m)); }
      else if (selectedElement.type === "drawing" && dragStartCoords) {
        const dx = x - dragStartCoords.x; const dy = y - dragStartCoords.y;
        setDrawings(prev => prev.map(d => d.id === selectedElement.id ? { ...d, points: originalPoints.map(p => ({ x: p.x + dx, y: p.y + dy })) } : d));
      }
    }
  };
  ...
  const handleWheel = (e: React.WheelEvent) => {
    if (!selectedMap) return;
    e.preventDefault();
    const zoomSpeed = 0.1; const minZoom = 0.5; const maxZoom = 5;
  ...
  return (
    <div style={{ color: "white", backgroundColor: "#111", position: "fixed", top: "60px", left: 0, height: "calc(100vh - 60px)", width: "100vw", display: "flex", overflow: "hidden", zIndex: 50 }}>
      ...
      {/* LEFT SIDEBAR: MAP & ASSET SELECTION */}
      <div style={{ 
        ...
        <div style={{ minWidth: "310px" }}>
          ...
          <div style={{ background: "#222", padding: "15px", borderRadius: "8px", border: "1px solid #444", marginBottom: "20px", opacity: selectedMap ? 1 : 0.5 }}>
          ...
          <div style={{ background: "#222", padding: "15px", borderRadius: "8px", border: "1px solid #444", opacity: selectedMap ? 1 : 0.5, display: "flex", flexDirection: "column", maxHeight: "380px" }}>
      ...
      {/* VIEWPORT: THE INTERACTIVE MAP */}
      <div 
        ref={mapRef} onWheel={handleWheel} onDragOver={(e) => e.preventDefault()} 
        onDrop={handleDrop} onClick={handleMapClick} onMouseDown={handleMouseDown}
        onMouseMove={handleMouseMove} onMouseUp={handleMouseUp}
        style={{ 
          flex: 1, display: "flex", backgroundColor: "#000000", 
          position: "relative", minWidth: 0, justifyContent: "center", 
          alignItems: "center", overflow: "hidden", 
          cursor: spacePressed || isPanning ? "grabbing" : selectedMap ? "default" : "not-allowed"
        }}
      >
        <button onClick={() => setSidebarOpen(!sidebarOpen)} style={{ position: "absolute", top: "20px", left: "10px", zIndex: 15, background: "rgba(230, 0, 130, 0.8)", border: "none", color: "white", borderRadius: "4px", width: "36px", height: "36px", cursor: "pointer", fontWeight: "bold" }}>{sidebarOpen ? "«" : "»"}</button>
        ...
        <div style={{ 
            ...
            pointerEvents: selectedMap ? "auto" : "none", opacity: selectedMap ? 1 : 0.6
          }}
        >
          {!selectedMap && (
              <div style={{ position: "absolute", top: "50%", left: "50%", transform: "translate(-50%, -50%)", color: "#f65dfb", zIndex: 10, textAlign: "center" }}>
                  <p style={{ fontSize: "20px", fontWeight: "bold" }}>← Please Select a Map to Start</p>
              </div>
          )}

          <canvas ref={canvasRef} width={1000} height={600} style={{ position: "absolute", top: 0, left: 0, width: "100%", height: "100%", cursor: activeTool === "pen" ? "crosshair" : activeTool === "eraser" ? "cell" : "default", zIndex: 2 }} />

          {markers.map(m => {
            const isSelected = selectedElement?.type === "marker" && selectedElement.id === m.id;
            return (
              <div key={m.id} style={{ position: "absolute", left: `${(m.x / 1000) * 100}%`, top: `${(m.y / 600) * 100}%`, width: "40px", height: "40px", transform: "translate(-50%, -50%)", zIndex: 5, pointerEvents: "none" }}>
                {m.type === "player" ? (
        ...
        {selectedMap && (
          <div style={{ position: "absolute", bottom: "20px", left: "20px", zIndex: 15, background: "rgba(0,0,0,0.6)", padding: "5px 10px", borderRadius: "4px", color: "#aaa", fontSize: "12px" }}>
            Zoom: {Math.round(zoom * 100)}% | Hold Space + Drag to Pan
          </div>
        )}

        <button onClick={() => setRightSidebarOpen(!rightSidebarOpen)} style={{ position: "absolute", top: "20px", right: rightSidebarOpen ? "25px" : "20px", zIndex: 15, background: "rgba(230, 0, 130, 0.8)", border: "none", color: "white", borderRadius: "4px", width: "36px", height: "36px", fontWeight: "bold" }}>{rightSidebarOpen ? "»" : "«"}</button>

        {rightSidebarOpen && (activeTool === "pen" || isSlidingBrush) && (
          <div style={{ position: "absolute", top: "65px", right: "20px", zIndex: 15, display: "flex", flexDirection: "column", alignItems: "center", gap: "5px", background: "#222", padding: "10px 6px", borderRadius: "8px", border: "1px solid #444" }}>
            <span style={{ fontSize: "11px", color: "#aaa" }}>Size</span>
            <input type="range" min="1" max="15" value={brushSize} onMouseDown={() => setIsSlidingBrush(true)} onMouseUp={() => setIsSlidingBrush(false)} onChange={(e) => setBrushSize(Number(e.target.value))} style={{ writingMode: "vertical-lr", direction: "rtl", appearance: "slider-vertical" as any, width: "15px", height: "100px", cursor: "pointer" }} />
        ...
        {rightSidebarOpen && (
          <>
            <div style={{ display: "flex", flexDirection: "column", gap: "10px", width: "100%", alignItems: "center" }}>
              <ToolButton name="select" icon={<SelectIcon />} activeTool={activeTool} onClick={(tool) => setActiveTool(activeTool === "select" ? null : tool)} disabled={!selectedMap} title="Select (S)" />
              <ToolButton name="pen" icon={<PenIcon />} activeTool={activeTool} onClick={(tool) => setActiveTool(activeTool === "pen" ? null : tool)} disabled={!selectedMap} title="Draw (P)" />
              <ToolButton name="eraser" icon={<EraserIcon />} activeTool={activeTool} onClick={(tool) => setActiveTool(activeTool === "eraser" ? null : tool)} disabled={!selectedMap} title="Erase (E)" />
            </div>
            <div style={{ height: "1px", background: "#333", width: "100%" }} />
            <div style={{ display: "flex", flexDirection: "column", gap: "10px" }}>
              <button onClick={handleUndo} disabled={historyIndex === 0} style={{ width: "50px", height: "50px", borderRadius: "8px", background: "#333", border: "none", color: "white", opacity: historyIndex === 0 ? 0.3 : 1 }}><UndoIcon /></button>
              <button onClick={handleRedo} disabled={historyIndex >= history.length - 1} style={{ width: "50px", height: "50px", borderRadius: "8px", background: "#333", border: "none", color: "white", opacity: historyIndex >= history.length - 1 ? 0.3 : 1 }}><RedoIcon /></button>
            </div>
            <div style={{ height: "1px", background: "#333", width: "100%" }} />
            <button onClick={deleteSelectedElement} disabled={!selectedElement} style={{ width: "50px", height: "50px", borderRadius: "8px", background: selectedElement ? "#c4302b" : "#333", border: "none", color: "white" }}><TrashIcon /></button>
          </>
        )}
      </div>
      ...
      {isMapSelectorOpen && (
        <div style={{ position: "fixed", top: 0, left: 0, width: "100vw", height: "100vh", backgroundColor: "rgba(0, 0, 0, 0.85)", display: "flex", justifyContent: "center", alignItems: "center", zIndex: 1000, backdropFilter: "blur(4px)" }}>
          <div style={{ background: "#161616", padding: "30px", borderRadius: "12px", border: "1px solid #282828", width: "800px", maxWidth: "90vw", maxHeight: "80vh", display: "flex", flexDirection: "column", position: "relative" }}>
            <button onClick={() => setIsMapSelectorOpen(false)} style={{ position: "absolute", top: "15px", right: "15px", background: "transparent", border: "none", color: "#666", fontSize: "24px", cursor: "pointer" }}>×</button>
              ...
              <div style={{ flex: "1 1 50%", overflowY: "auto" }}>
                <div style={{ marginBottom: "20px" }}>
                  ...
                  <div style={{ display: "flex", gap: "8px", flexWrap: "wrap" }}>
                    {gameModes.map((mode) => (
                      <button key={mode} onClick={() => { setSelectedMode(selectedMode === mode ? null : mode); setSelectedMap(null); }} style={{ padding: "8px 12px", background: selectedMode === mode ? "#e60082" : "#333", color: "white", border: "none", borderRadius: "4px", cursor: "pointer" }}>{mode}</button>
                    ))}
                  </div>
                </div>
                <div style={{ opacity: selectedMode ? 1 : 0, pointerEvents: selectedMode ? "auto" : "none" }}>
                  <h4 style={{ margin: "0 0 10px 0", color: "#aaa" }}>2. Choose Map</h4>
                  <div style={{ display: "flex", flexWrap: "wrap", gap: "8px" }}>
                    {filteredMaps.map((map) => (
                      <button key={map.map_id} onMouseEnter={() => setHoveredMap(map)} onMouseLeave={() => setHoveredMap(null)} onClick={() => { setSelectedMap(map.name); setIsMapSelectorOpen(false); }} style={{ padding: "6px 10px", background: selectedMap === map.name ? "#f65dfb" : "#444", color: "white", border: "none", borderRadius: "4px", cursor: "pointer" }}>{map.name}</button>
                    ))}
                  </div>
                </div>
              </div>
              <div style={{ width: "350px", height: "350px", background: "#111", borderRadius: "8px", border: "1px solid #333", display: "flex", justifyContent: "center", alignItems: "center", overflow: "hidden" }}>
                {hoveredMap?.image_path ? <img src={hoveredMap.image_path} alt={hoveredMap.name} style={{ width: "100%", height: "100%", objectFit: "cover" }} /> : <div style={{ color: "#555" }}>Hover to preview</div>}
              </div>
            </div>
          </div>
        </div>
      ...
      <CustomModal isOpen={isDuplicateNameModalOpen} title="Name Already Exists" onConfirm={() => { setIsDuplicateNameModalOpen(false); setIsNameModalOpen(true); }} showCancel={false} confirmText="Try Different Name">
        <p style={{ color: "#aaa" }}>You already have a strategy named "<strong>{newStrategyName}</strong>". Please choose a unique name to save this plan.</p>
      </CustomModal>
      ...
      <CustomModal isOpen={isResetModalOpen} title="Reset Map?" onConfirm={clearCanvas} onCancel={() => setIsResetModalOpen(false)} confirmText="Reset">
        <p style={{ color: "#aaa" }}>This will clear all markers and drawings. Are you sure?</p>
      </CustomModal>

      <CustomModal isOpen={isAuthModalOpen} title="Not Signed In" onConfirm={() => navigate("/login")} confirmText="Log In">
        <p style={{ color: "#aaa" }}>Sign in or log back in to save maps.</p>
      </CustomModal>
    </div>
  );
};
